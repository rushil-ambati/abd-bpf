...
/// This crate has a runtime dependency on artifacts produced by the `ebpf-actors-ebpf` crate.
...
fn main() {
    ...
    let ebpf_package = packages
        .into_iter()
        .find(|Package { name, .. }| name == "ebpf-actors-ebpf")
        .unwrap();
    ...
    if build_ebpf {
        ...
        // We have a build-dependency on `ebpf-actors-ebpf`, so cargo will automatically rebuild us
        // if `ebpf-actors-ebpf`'s *library* target or any of its dependencies change. Since we
        // depend on `ebpf-actors-ebpf`'s *binary* targets, that only gets us half of the way. This
        ...
        let ebpf_target_dir = out_dir.join("ebpf-actors-ebpf");
