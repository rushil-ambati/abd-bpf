...
fn validate_write_response(buf: &[u8]) -> BenchmarkResult<()> {
    ...
    match AbdMessageType::try_from(resp.type_) {
        ...
        Ok(other) => Err(BenchmarkError::Protocol(format!(
            "Expected WriteAck, received {:?}",
            other
        ))),
        Err(_) => Err(BenchmarkError::Protocol(format!(
        ...
        Ok((n, _)) => {
            ...
            if let Ok(archived) = access::<ArchivedAbdMessage, RkyvError>(&buf[..n]) {
                if let Ok(resp) = deserialize::<AbdMessage, RkyvError>(archived) {
                    Ok(AbdMessageType::try_from(resp.type_) == Ok(AbdMessageType::WriteAck))
                } else {
                    Ok(false) // Deserialization failed
                }
            } else {
                Ok(false) // Invalid message format
            }
        }
        Err(ref e)
